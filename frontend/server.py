...
def do_classification(image):
    places_prob, weight_softmax, idx = classify_places(io.BytesIO(image))
    objects_prob = classify_objects(io.BytesIO(image))
...
def classify():
    ...
    # do the stuff here
    obj_dist, plc_dist = do_classification(io.BytesIO(image))
